...
The model produces a MSE test score of 29930845 suggesting a linear model may not be a good fit for the data. This poses me to question the assumed linearity within the dataset, and to consider alternative  approaches like the General Additive Model (GAM).
...
My next model is a regression tree that prioritize simplicity and interpretability. It is primarily concerned with the variables ‘smoker’, ‘age’, and ‘bmi’ that hold leading coefficients as evident in the regression tree plot attached below; overlooking other variables that may hold similar significance.
